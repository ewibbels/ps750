*! Version 1.10 1/5/04 David Roodman droodman@cgdev.org
*
...
/* now check for perfect collinearity in instrument list */
		...
		forvalues i = 1/`endo_ct' {
			if "`first'"!="" {
				regress ``i'' `exog' `newexog' `wtexp' if `touse', `noconstant'
			}
			else {
				quietly regress ``i'' `exog' `newexog' `wtexp' if `touse', `noconstant'
			}
			tempvar `i'b
			ren ``i'' ``i'b'

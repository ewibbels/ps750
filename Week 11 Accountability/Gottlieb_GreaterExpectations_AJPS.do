...
cd "C:\Users\phant\Dropbox\Coursework\Spring 2017\Wibbels\Replication\Gottlieb"
...
////////EXTENSION: P(PICKING GOOD CANDIDATE) ~ TREATMENT
destring avote, replace force
lab var avote "Voted for the public goods candidate"
lab var t1 "Treatment 1"
lab var t2 "Treatment 2"

set more off
xtmixed avote t1 t2 i.block i.enumerator, ||cid: ||village:
outreg2 using ext1, label word replace keep(t1 t2)

recode avote 1=0 2=1, gen(avote1)
probit avote1 t1 t2 i.block i.enumerator
...
/////EXTENSION: P(VOTE B) ~ TREAMENT | DIFFERENT TYPES OF B

preserve
keep if btype==1
xtmixed bvote t1 t2 i.block i.enumerator, ||cid: ||village:
outreg2 using ext2, word replace label keep(t1 t2)
restore
...
keep if btype==2
xtmixed bvote t1 t2 i.block i.enumerator, ||cid: ||village:
outreg2 using ext2, word append label keep(t1 t2)
restore
